...
public class UtilsManager extends UtilsAdmob {
    ...
    private  void more_games(){
        try {
            String url_more_games = activity.getResources().getString(R.string.more_games);
            if (url_more_games.isEmpty()) {
                activity.startActivity(new Intent(Intent.ACTION_VIEW,
                        Uri.parse("https://play.google.com/store/apps/details?id=" + activity.getApplication().getPackageName())));
            }
            else{
                activity.startActivity(new Intent(Intent.ACTION_VIEW, Uri.parse(url_more_games)));
            }
        }
        catch (Exception e){
